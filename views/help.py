...
class View(discord.ui.View):
	...
	def add_buttons(self):
		self.startB = Buttons(emoji="<:backward:959059561778655332>", style=discord.ButtonStyle.grey, command=self.set_page, args=0, ctx = self.ctx)
		self.backB = Buttons(emoji="<:back1:959059484867698738>", style=discord.ButtonStyle.blurple, command=self.to_page, args=-1, ctx = self.ctx)
		self.nextB = Buttons(emoji="<:next1:959059275915857930>", style=discord.ButtonStyle.blurple, command=self.to_page, args=+1, ctx = self.ctx)
		self.endB = Buttons(emoji="<:Forward1:959059148417409024>", style=discord.ButtonStyle.grey, command=self.set_page, args=len(self.options)-1, ctx = self.ctx)
		self.quitB = Buttons(emoji="<:stop1:959059385345261628>", style=discord.ButtonStyle.red, command=self.quit, ctx = self.ctx)
	...
	def gen_embeds(self):
		...
		for cog in self.get_cogs():
			if "help_custom" in dir(cog):
				...
				embed = discord.Embed(title = f"{emoji} Help · {label}",description=f"`{cog.__doc__}`", url="https://dsc.gg/flamer-hq")
