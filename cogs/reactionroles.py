...
try:
    with open("./data/databases/reaction_roles.json") as file:
...
def store_reaction_roles():
    with open("./data/databases/reaction_roles.json", "w") as file:
...
class ReactionRoles(commands.Cog):
    ...
    def help_custom(self):
		      emoji = '<a:zz_star:958709570241433632>'
		      label = "ReactionRoles"
		      description = "Shows all RR Commands"
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        ...
        if role is not None and user is not None:
            await user.add_roles(role, reason="Flamer ReactionRole")
    ...
    async def reaction(self, ctx, emote, role: discord.Role, channel: discord.TextChannel, *, title=None):
        if title is None:
